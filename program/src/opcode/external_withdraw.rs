...
/*
    This instruction is used to withdraw tokens from a virtual account to a real
    (external) token account. After the withdrawal, the account is deleted. The
    signature of the source account is required to authorize the withdraw.
...
pub fn process_external_withdraw(
    ...
    let args = ExternalWithdrawOp::try_from_bytes(&data.data)?;
