...
use anchor_spl::token_2022::spl_token_2022::solana_zk_token_sdk::curve25519::{
    edwards::{
        add_edwards, 
        multiply_edwards,
        PodEdwardsPoint
...
pub fn sig_verify(pubkey: &[u8], sig: &[u8], message: &[u8]) -> Result<()> {
    ...
    if pubkey.len() != ED25519_PUBKEY_LEN {
        return Err(CodeVmError::InvalidSignature.into());
    }
    ...
    */
    ...
    if expected_R == computed_R {
        ...
        Err(CodeVmError::InvalidSignature.into())
    }
}
