...
impl <const NUM_PAGES: usize, const PAGE_SIZE: usize> 
    ...
    fn try_find_empty(&self, num_required: usize) -> Result<Vec<u8>> {
        ...
        if empty.len() < num_required {
            return Err(CodeVmError::MemorySectorInsufficientPages.into());
        }
    ...
    pub fn try_alloc_pages(&mut self, data_size: usize) -> Result<u8> {
        let num_required = Page::<PAGE_SIZE>::calc_num_pages_needed(data_size);
        if num_required > self.get_num_empty() as usize {
            return Err(CodeVmError::MemorySectorOutOfMemory.into());
        }
